...
while True:
    ...
    # 스테이지 모드의 경우 선후공 없이 1, 3, 5 번공을 넣으면 된다
    # 따라서 1번공이 없다면 3번을, 3번도 없다면 5번 공을 타겟볼로 지정
    if ord == 1:    
    ...
    if cnt >= 1:
        for i in range(6):
            w = abs(HOLES[i][0] - targetBall_x)
            h = abs(HOLES[i][1] - targetBall_y)
            if min > w + h:
                min = w + h
                hole_x = HOLES[i][0]
                hole_y = HOLES[i][1]
        ...
        ga = math.atan(abs(hole_x - whiteBall_x) / abs(hole_y - whiteBall_y))
        ga2 = 180 / math.pi * ga
        da = math.acos((a ** 2 + b ** 2 - c ** 2) / (2 * a * b))
        da2 = 180 / math.pi * da
        d = math.sqrt((b + r) ** 2 + a ** 2 - (2 * a * (b + r) * math.cos(da2)))
        na = math.acos((a ** 2 + d ** 2 - (b + r) ** 2) / (2 * a * d))
        na2 = 180 / math.pi * na
        print(a, b, c, na2)
        angle = ga2 + na2 / 2
